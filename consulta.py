...
#             # Agrupar por Nombre y sumar la Cantidad_Movida
#             grouped_df = df.groupby('Nombre').agg({'Tipo_Movimiento': 'first', 'Cantidad_Movida': 'sum', 'Fecha_Hora_Movimiento': 'first', 'Razon_Movimiento': 'first'}).reset_index()
#             st.write(grouped_df)
#         else:
#             st.warning('No se encontraron resultados para los filtros seleccionados.')
#     else:
...
#             st.dataframe(df)
#             df.set_index('Fecha_Hora_Movimiento', inplace=True)
#             df['Cantidad_Movida'].plot(kind='bar')
#             plt.title('Cantidad movida por fecha')
#             plt.xlabel('Fecha')
